...
async function createServer({ httpServer, httpsServer, ip, port, setSystemProxy }) {
  ...
  proxyLogger.info('proxy server listen on %s port %s', addr.address, addr.port);
  if (setSystemProxy) {
    await setProxy(addr.address, addr.port);
    console.log('System proxy has been set to %s:%s', addr.address, addr.port);
    proxyLogger.info('System proxy has been set to %s:%s', addr.address, addr.port);
  }
  ...
  process.on('SIGINT', async () => {
    if (setSystemProxy) {
      ...
      proxyLogger.info('System proxy has been unset');
    }
    process.exit();
  });
}
