...
async function createWebServers(ca, rules) {
  ...
  const httpServerAddress = await portfinder.getPortPromise({ startPort: 8000 });
  await createHttpServer(app, httpPort);
  const httpsServerAddress = await portfinder.getPortPromise({ startPort: 8000 });
...
async function createHttpServer(app) {
  const httpPort = await portfinder.getPortPromise({ startPort: 8000 });
...
async function createHttpsServer(app, hostname) {
  const httpsPort = await portfinder.getPortPromise({ startPort: 8000 });
