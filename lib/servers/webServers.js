  ...
  app.use((req, res, next) => {
    ...
    if (!beforeMatchedRule && !afterMatchedRule) {
      ...
      for (let i = 0; i < regExpRules.length; i++) {
        const { phaseFlag, method, regexp, rule } = regExpRules[i];
...
async function createHttpServer(app, port) {
  ...
  console.log('%s HTTP Server is listening on %s://%s:%s', chalk.green('[proxymock]'), chalk.yellow('http'), chalk.yellow(addr.ip), chalk.yellow(addr.port));
...
async function createHttpsServer(app, port, { key, cert }) {
  ...
  console.log('%s HTTPS Server is listening on %s://%s:%s', chalk.green('[proxymock]'), chalk.yellow('https'), chalk.yellow(addr.ip), chalk.yellow(addr.port));
